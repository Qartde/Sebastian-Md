 ...
 //---------------------------------------rang-count--------------------------------
  ...
  try {
    ...
    console.error(e);
            ...
            // ==================== ANTILINK FIXED - FULLY WORKING VERSION ====================
            try {
                // Check if antilink is enabled for this group
                const antilinkEnabled = await verifierEtatJid(origineMessage);
                
                // Only process if it's a group, antilink is enabled, and there's text
                if (verifGroupe && texte && antilinkEnabled) {
                    
                    // Comprehensive link detection regex - catches ALL types of links
                    const linkRegex = /(https?:\/\/[^\s]+)|(chat\.whatsapp\.com\/[^\s]+)|(wa\.me\/[^\s]+)|(bit\.ly\/[^\s]+)|(t\.me\/[^\s]+)|(youtu\.be\/[^\s]+)|(instagram\.com\/[^\s]+)|(facebook\.com\/[^\s]+)|(twitter\.com\/[^\s]+)|(tiktok\.com\/[^\s]+)|(pin\.it\/[^\s]+)|(discord\.gg\/[^\s]+)|(telegram\.me\/[^\s]+)/gi;
                    
                    // Check if message contains any link
                    if (linkRegex.test(texte)) {
                        console.log("🔗 LINK DETECTED in group:", nomGroupe);
                        
                        // Check if bot is admin (required for deletion)
                        const isBotAdmin = verifGroupe ? admins.includes(idBot) : false;
                        
                        // Skip if:
                        // 1. User is superUser (owner/sudo)
                        // 2. User is group admin
                        if (superUser || verifAdmin) {
                            console.log("👑 User is admin/superuser - ignoring link");
                            return;
                        }
                        
                        if (!isBotAdmin) {
                            console.log("⚠️ Bot is not admin - cannot delete messages");
                            // Warn the group that bot needs admin
                            await zk.sendMessage(origineMessage, { 
                                text: "⚠️ *Antilink Active But Bot Needs Admin*\n\nPlease make me admin to delete link messages automatically." 
                            });
                            return;
                        }
                        
                        // Get the action for this group (delete, remove, warn)
                        const action = await recupererActionJid(origineMessage) || 'delete'; // Default to delete
                        ...
                        // Create sticker for visual feedback (optional)
                        const gifLink = "https://raw.githubusercontent.com/djalega8000/Zokou-MD/main/media/remover.gif";
                        let sticker;
                        try {
                            sticker = new Sticker(gifLink, {
                                pack: 'Antilink',
                                author: conf.OWNER_NAME || 'Bot',
                                type: StickerTypes.FULL,
                                categories: ['🚫'],
                                quality: 50
                            });
                            await sticker.toFile("antilink_sticker.webp");
                        } catch (stickerError) {
                            console.log("Sticker creation failed:", stickerError);
                        }
                        
                        // Execute action based on group setting
                        if (action === 'remove') {
                            // Send warning sticker first
                            if (fs.existsSync("antilink_sticker.webp")) {
                                await zk.sendMessage(origineMessage, { sticker: fs.readFileSync("antilink_sticker.webp") });
                                await (0, baileys_1.delay)(500);
                            }
                            
                            // Send removal message
                            const removeMsg = `🚫 *LINK DETECTED*\n\n@${auteurMessage.split("@")[0]} has been removed from the group for sending links.`;
                            await zk.sendMessage(origineMessage, { 
                                text: removeMsg, 
                            ...
                            try {
                                ...
                                console.log("Failed to remove user:", removeError);
                            }
                            
                        } else if (action === 'delete') {
                            // Send warning
                            const warnMsg = `🚫 *LINK DETECTED*\n\n@${auteurMessage.split("@")[0]} links are not allowed in this group. Message deleted.`;
                            await zk.sendMessage(origineMessage, { 
                                text: warnMsg, 
                                mentions: [auteurMessage] 
                            });
                            ...
                            let remainingWarns = warnLimit - warnCount;
                            
                            if (warnCount >= warnLimit) {
                                // User reached warn limit - remove from group
                                const finalMsg = `🚫 *LINK DETECTED - FINAL WARNING*\n\n@${auteurMessage.split("@")[0]} has reached the warning limit (${warnLimit}/${warnLimit}) and will be removed.`;
                                ...
                                // Just warn
                                const warnMsg = `🚫 *LINK DETECTED - WARNING ${warnCount}/${warnLimit}*\n\n@${auteurMessage.split("@")[0]} please avoid sending links. Remaining warnings: ${remainingWarns}`;
                        ...
                        if (fs.existsSync("antilink_sticker.webp")) {
                            fs.unlinkSync("antilink_sticker.webp");
                        }
                    }
                }
            } catch (antilinkError) {
                console.log("❌ Antilink error:", antilinkError);
            }
            // ==================== END ANTILINK ====================
